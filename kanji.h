...
class kanji{ //TODO: add more info about each kanji (grammatical use(noun, verb, adverb etc etc), defination/translation, common words using that kanji(?))
    public:
        kanji(std::vector<std::wstring> , std::vector<std::wstring> , std::wstring); //set both readings on init
        void setOn(std::vector<std::wstring> ); //set on readings
        void setKun(std::vector<std::wstring> ); //set kun readings
    ...
    private:
        std::vector<std::wstring> on; //output
        std::vector<std::wstring> kun; //input
};
